...
func TestUploadAnnexAndCleanup(t *testing.T) {
	env := newTestEnv(t)
	...
	// 清理孤儿：上传后未引用 → 被清掉（含磁盘文件）
	rec, cleanupOut := doJSON(t, env, "POST", "/api/annex/cleanup", nil)
	if rec.Code != http.StatusOK {
		t.Fatalf("清理失败: %d %s", rec.Code, rec.Body.String())
	}
	if n := asInt(asMap(cleanupOut)["count"]); n != 1 {
		t.Fatalf("期望清理 1 个孤儿，得到 %v", asMap(cleanupOut)["count"])
	}
	entries, _ = os.ReadDir(env.annexDir)
	if len(entries) != 0 {
		t.Fatalf("清理后附件目录应为空，得到 %d 个文件", len(entries))
	}
}

func TestReferencedAnnexSurvivesCleanup(t *testing.T) {
	env := newTestEnv(t)
	rec, out := doJSON(t, env, "POST", "/api/rooms", model.RoomInput{Name: "一号配电室"})
	if rec.Code != http.StatusOK {
		t.Fatalf("创建房间失败")
	...
	// 上传后被房间引用 → cleanup 后应保留
	rec, up := uploadAnnex(t, env, "room.png", tinyPNG)
	if rec.Code != http.StatusOK {
	...
	rec, out = doJSON(t, env, "PUT", "/api/rooms/"+strconv.FormatInt(roomID, 10), model.RoomInput{
		Name: "一号配电室", ImageIDs: []int64{annexID},
	})
	if rec.Code != http.StatusOK {
	...
	rec, out = doJSON(t, env, "POST", "/api/annex/cleanup", nil)
	if rec.Code != http.StatusOK {
		t.Fatalf("清理失败: %d", rec.Code)
	}
	if n := asInt(asMap(out)["count"]); n != 0 {
		t.Fatalf("被引用附件不应被清理，得到 %v", asMap(out)["count"])
	}
	entries, _ := os.ReadDir(env.annexDir)
	if len(entries) != 1 {
		t.Fatalf("附件目录应保留 1 个文件，得到 %d", len(entries))
	}
}
